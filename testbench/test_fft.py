...
# JIT compile the FFT and inverse FFT kernels
@catapult.jit(kernel_path="cu/fft.cu", kernel_name="fft")
def fft_kernel(kernel, blk, N, din, dcin, size):
    threads_per_block = min(size, 1024)
    blocks_per_grid = 1  # Must match the original launch

    print(f"Launching FFT kernel: grid=({blocks_per_grid},1,1), block=({threads_per_block},1,1)")

    kernel[(blocks_per_grid, 1, 1), (threads_per_block, 1, 1)](din, dcin, size)

@catapult.jit(kernel_path="cu/fft.cu", kernel_name="invfft")
def ifft_kernel(kernel, blk, N, din, dcin, size):
    threads_per_block = min(size, 1024)
    blocks_per_grid = 1  # Must match the original launch
...
def test_fft_correctness(cuda_device, N):
    """Test FFT kernel correctness against PyTorch's FFT implementation."""
    if (N & (N - 1)) != 0:
...
def test_fft_inverse(cuda_device, N):
    """Test FFT + Inverse FFT returns to original input."""
    if (N & (N - 1)) != 0:
        raise ValueError(f"N must be a power of two! Got N={N}")
...
def benchmark_fft(cuda_device):
    """Measure FFT execution time for increasing input sizes and generate a graph."""
