...
template<int _d_model, float dropout_val> struct norm_globals {
    static constexpr int d_model = _d_model;
    static constexpr float dropout_p = dropout_val;
    ...
    const int n_tile_size = x.depth() / 2;
    const int n_per_tile = 2;

    dim3 grid() const {
        return dim3(x.depth() / 2, x.batch(), 1);
    }
    dim3 block() const {
    ...
    static constexpr auto inputs() {
        return std::make_tuple(
            &norm_globals<_d_model, dropout_val>::x,
            &norm_globals<_d_model, dropout_val>::residual,
            &norm_globals<_d_model, dropout_val>::o,
            &norm_globals<_d_model, dropout_val>::o_resid,
            &norm_globals<_d_model, dropout_val>::norm_weight,
            &norm_globals<_d_model, dropout_val>::norm_bias
        );
    }
};
...
// Uncomment and fix the dispatch_layernorm function if needed:
/*
void dispatch_layernorm(
    ...
    constexpr float dropout_val = 0.1f;
    ...
    // global pointers
    using globals = norm_globals<D, dropout_val>;
    ...
    cudaFuncSetAttribute(
        layernorm_tk<D, dropout_val>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        mem_size
    );

    dim3 grid(n_tile_size, B, 1);
    layernorm_tk<D, dropout_val><<<grid,NUM_THREADS,mem_size>>>(g);
    cudaDeviceSynchronize();
}
*/

// #ifdef TK_COMPILE_FUSED_LAYERNORM #include "pyutils/torch_helpers.cuh"
// #include <iostream>
