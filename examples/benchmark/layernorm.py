...
def eager_layernorm(x, residual, norm_weight, norm_bias, B, N, D, dropout_p=0.0):
    x = x + residual
    o_resid = x
...
def torch_layernorm(x, residual, norm_weight, norm_bias, B, N, D, dropout_p=0.0):
    x = x + residual
    o_resid = x
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[32 * i for i in range(2, 28)],  # different possible values for `x_name`
...
def benchmark(B, N, D, provider):
    ...
    if provider == 'eager':
        ms = triton.testing.do_bench(lambda: eager_layernorm(x, residual, norm_weight, norm_bias, B, N, D), return_mode='median', warmup=100, rep=500)
    if provider == 'torch':
        ms = triton.testing.do_bench(lambda: torch_layernorm(x, residual, norm_weight, norm_bias, B, N, D), return_mode='median', warmup=100, rep=500)
